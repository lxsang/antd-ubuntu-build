...
function DBModel:get(id)
	return sqlite.select(self.name,"id="..id)[0]
end

function DBModel:getAll()
	local sql = "SELECT * FROM "..self.name
	return sqlite.select(self.name, "1=1")
end

function DBModel:find(cond)
	return sqlite.select(self.name, cond)
end
...
function DBModel:available()
	return sqlite.hasTable(self.name) == 1
end
