...
(function() {
  ...
  wTerm = (function(superClass) {
    extend(wTerm, superClass);

    function wTerm(args) {
      wTerm.__super__.constructor.call(this, "wTerm", args);
    }

    wTerm.prototype.main = function() {
      var area, me;
      me = this;
      ...
      this.term.on("key", function(d, e) {
        if (me.socket) {
          return me.socket.send("i" + d);
        }
      });
      this.term.on('title', function() {
        return console.log("title change");
      });
      ...
      area = ($(".xterm-helper-textarea", this.mterm))[0];
      area.onpaste = function(e) {
        var chunklen, end, i, j, len, pastedText, ref, results;
        pastedText = void 0;
        ...
        chunklen = len / 1000 + (len % 1000 === 0 ? 0 : 1);
        results = [];
        for (i = j = 0, ref = len - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
          end = (i + 1) * 1000 > len ? len : (i + 1) * 1000;
          results.push(me.term.write(pastedText.substring(i * 1000, end)));
        }
        return results;
      };
      ...
      this.openSession();
      return this.on("hboxchange", function(e) {
        return me.resizeContent(e.w, e.h);
      });
    };

    wTerm.prototype.resizeContent = function(w, h) {
      var ex, ncol, nrow, oldhtml;
      ex = this.term.rowContainer.firstElementChild;
      oldhtml = ($(ex)).html();
    ...
    wTerm.prototype.cleanup = function(e) {
      if (this.socket) {
