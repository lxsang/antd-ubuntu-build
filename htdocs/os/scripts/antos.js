...
(function() {
  var AboutDialog, ApplicationHandler, BaseApplication, BaseDialog, BaseEvent, BaseFileHandler, BaseModel, BaseService, BasicDialog, BlobFileHandler, CalendarDialog, ColorPickerDialog, DB, FileDiaLog, G_CACHE, GoogleDriveHandler, InfoDialog, PromptDialog, RemoteFileHandler, SelectionDialog, SharedFileHandler, SubWindow, YesNoDialog, _API, _GUI, _OS, _PM, _REST, _courrier, self,
  ...
  self.OS.API = {
    ...
    requires: function(libs, f) {
      if (!(libs.length > 0)) {
        return f();
      }
      _courrier.observable.one("sharedlibraryloaded", function(l) {
        libs.splice(0, 1);
        return _API.requires(libs, f);
      });
      return _API.require(libs[0], null);
    },
    packages: {
      fetch: function(f) {
  ...
  String.prototype.asFileHandler = function() {
    ...
    if (!handlers || handlers.length === 0) {
      _courrier.osfail("VFS unknown handler: " + this, _API.throwe("OS.VFS"), this);
      return null;
    }
    return new handlers[0](this);
  };

  this.OS.API.VFS = {
    handlers: {},
    register: function(protos, cls) {
      return self.OS.API.VFS.handlers[protos] = cls;
    },
    findHandlers: function(proto) {
      var k, l, v;
      l = (function() {
        var ref, results;
        ref = _API.VFS.handlers;
        results = [];
        for (k in ref) {
          v = ref[k];
  ...
  BaseFileHandler = (function() {
    function BaseFileHandler(path) {
      this.dirty = false;
      this.cache = void 0;
      this.setPath(path);
    }

    BaseFileHandler.prototype.setPath = function(p) {
      var list, re;
      this.ready = false;
      if (!p) {
        return;
    ...
    BaseFileHandler.prototype.isRoot = function() {
    ...
    BaseFileHandler.prototype.hash = function() {
      if (!this.path) {
        return -1;
      }
    ...
    BaseFileHandler.prototype.onready = function(f, err) {
      var me;
      if (this.ready) {
        return f();
      }
      ...
      return me.meta(function(d) {
        if (d.error) {
          if (err) {
            ...
            return _courrier.osfail(me.path + ": " + d.error, _API.throwe("OS.VFS"), d.error);
          }
        }
        me.info = d.result;
        me.ready = true;
    ...
    BaseFileHandler.prototype.read = function(f, t) {
      var me;
      me = this;
    ...
    BaseFileHandler.prototype.write = function(d, f) {
      var me;
      me = this;
    ...
    BaseFileHandler.prototype.mk = function(d, f) {
      var me;
      me = this;
      return this.onready((function() {
    ...
    BaseFileHandler.prototype.remove = function(f) {
      var me;
      me = this;
      return this.onready((function() {
    ...
    BaseFileHandler.prototype.upload = function(f) {
      var me;
      me = this;
      return this.onready((function() {
    ...
    BaseFileHandler.prototype.publish = function(f) {
      var me;
      me = this;
      return this.onready((function() {
        return me.action("publish", null, function(r) {
          if (r.result) {
            _courrier.ostrigger("VFS", {
              m: "publish",
              file: me
    ...
    BaseFileHandler.prototype.download = function(f) {
      var me;
      me = this;
      return this.onready((function() {
    ...
    BaseFileHandler.prototype.move = function(d, f) {
      var me;
      me = this;
      return this.onready((function() {
    ...
    BaseFileHandler.prototype.execute = function(f) {
      var me;
      me = this;
      return this.onready((function() {
    ...
    BaseFileHandler.prototype.meta = function(f) {};

    BaseFileHandler.prototype.action = function(n, p, f) {
  ...
  })(self.OS.API.VFS.BaseFileHandler);
  ...
  BlobFileHandler = (function(superClass) {
    extend(BlobFileHandler, superClass);

    function BlobFileHandler(path, mime, data) {
      BlobFileHandler.__super__.constructor.call(this, path);
      if (data) {
        this.cache = data;
      }
      this.info = {
    ...
    BlobFileHandler.prototype.meta = function(f) {
      return f();
    };

    BlobFileHandler.prototype.onchange = function(f) {
      return this.onchange = f;
    };

    BlobFileHandler.prototype.action = function(n, p, f) {
      var blob, me;
      me = this;
      switch (n) {
        case "read":
    ...
    return BlobFileHandler;

  })(self.OS.API.VFS.BaseFileHandler);

  self.OS.API.VFS.register("^blob$", BlobFileHandler);

  SharedFileHandler = (function(superClass) {
    extend(SharedFileHandler, superClass);

    function SharedFileHandler(path) {
      SharedFileHandler.__super__.constructor.call(this, path);
      if (this.isRoot()) {
        this.ready = true;
      }
    }

    SharedFileHandler.prototype.meta = function(f) {
      return _API.handler.fileinfo(this.path, f);
    };

    SharedFileHandler.prototype.action = function(n, p, f) {
      var me;
      me = this;
      switch (n) {
        case "read":
          if (this.isRoot()) {
            return _API.get(_API.handler.shared + "/all", f, (function(e, s) {}));
          }
          return _API.handler.readfile(this.path, f, p ? p : "text");
        case "mk":
          break;
  ...
  GoogleDriveHandler = (function(superClass) {
    ...
    GoogleDriveHandler.prototype.fields = function() {
  ...
  })(this.OS.API.VFS.BaseFileHandler);
  ...
  DB = (function() {
    ...
    DB.prototype["delete"] = function(c, f) {
      var rq;
      rq = {
        table: this.table
      };
      if (!(c && c !== "")) {
        return _courrier.oserror("Unknown condition for delete command", _API.throwe("OS.DB"), c);
      }
      if (isNaN(c)) {
        rq.cond = c;
      } else {
      ...
      return _API.handler.dbquery("delete", rq, f);
    };
  ...
  self.OS.GUI = {
    ...
    pushServices: function(srvs) {
      ...
      _courrier.observable.one("srvroutineready", function() {
        srvs.splice(0, 1);
        return _GUI.pushServices(srvs);
      });
      return _GUI.pushService(srvs[0]);
    },
    ...
    forceLaunch: function(app, args) {
      ...
      _PM.killAll(app);
      if (_OS.APP[app] && _OS.APP[app].style) {
        ($(_OS.APP[app].style)).remove();
      }
      _OS.APP[app] = void 0;
    ...
    login: function() {
      ...
      return _API.resource("schemes/login.html", function(x) {
        ...
        return ($("#txtpass")).keyup(function(e) {
          if (e.which === 13) {
  ...
  BaseModel = (function() {
    ...
    BaseModel.prototype.path = function() {
      var mt;
      mt = this.meta();
      if (mt && mt.path) {
        return mt.path;
  ...
  BasicDialog = (function(superClass) {
    ...
    BasicDialog.prototype.init = function() {
      ...
      for (k in ref) {
        v = ref[k];
        html += "<" + v.tag + " " + v.att + " style = 'margin-left:5px; margin-right:5px;' data-id = 'content" + k + "'></" + v.tag + ">";
      }
  ...
  PromptDialog = (function(superClass) {
    ...
    function PromptDialog() {
      PromptDialog.__super__.constructor.call(this, "PromptDialog", {
        ...
        xtra: function(d) {
          return $(d.find("content1")).keyup(function(e) {
            if (e.which === 13) {
  ...
  FileDiaLog = (function(superClass) {
    ...
    FileDiaLog.prototype.main = function() {
      ...
      (this.find("bt-ok")).set("onbtclick", function(e) {
        var d, f, j, len, m, ref, v;
        f = fileview.get("selectedFile");
        if (!f) {
          return me.notify("Please select a file");
        }
        if (me.data && me.data.mimes) {
          m = false;
          ref = me.data.mimes;
          for (j = 0, len = ref.length; j < len; j++) {
            v = ref[j];
          ...
          if (!m) {
            return me.notify("Only " + (me.data.mimes.join(",")) + " could be selected");
          }
        }
        d = f.path;
        if (f.type === "file") {
        ...
        if (me.handler) {
          me.handler(d, ($(filename)).val(), f.path);
        }
        return me.quit();
      });
